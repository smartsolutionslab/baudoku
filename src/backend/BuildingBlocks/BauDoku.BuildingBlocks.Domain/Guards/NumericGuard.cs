...
public readonly ref struct NumericGuard<T> where T : struct, INumber<T>
{
    ...
    {
        if (value < T.Zero) throw new ArgumentOutOfRangeException(paramName, message ?? $"{paramName} darf nicht negativ sein.");
        return this;
    }
    ...
    {
        if (value < min || value > max) throw new ArgumentOutOfRangeException(paramName, message ?? $"{paramName} muss zwischen {min} und {max} liegen.");
        return this;
    }
}
