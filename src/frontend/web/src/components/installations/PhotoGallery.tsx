...
export function PhotoGallery({ photos, onDelete }: PhotoGalleryProps) {
  ...
  if (photos.length === 0) {
    return <p className="py-8 text-center text-sm text-gray-500">Noch keine Fotos vorhanden.</p>;
  }
    ...
    <>
      ...
      {viewPhoto && (
        ...
        >
          ...
          />
          {viewPhoto.caption && (
            <p className="absolute bottom-6 text-center text-sm text-white">{viewPhoto.caption}</p>
          )}
        </div>
      )}
    </>
