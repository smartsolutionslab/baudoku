...
import { StatusBadge, buttonClassName } from '@/components/common';

export function DashboardPage() {
  const { data } = useProjects();

  const projects = useMemo(() => data?.pages.flatMap((p) => p.items) ?? [], [data]);
  ...
  return (
    <div>
      ...
      <p className="mt-1 text-sm text-gray-500">Willkommen bei BauDoku</p>

      {/* Stats */}
      <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        <DashboardCard title="Aktive Projekte" value={activeCount} color="blue" />
        <DashboardCard title="Abgeschlossen" value={completedCount} color="green" />
        <DashboardCard title="Gesamt" value={totalCount} color="gray" />
      </div>
          ...
          <div className="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {projects.slice(0, 6).map((project) => (
              ...
              >
                ...
                {project.city && <p className="mt-1 text-sm text-gray-500">{project.city}</p>}
